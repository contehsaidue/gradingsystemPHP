...
if(isset($_POST['adddata'])){
    ...
    $role = $_POST['role'];
      ...
      $root_path = "../assets/images/reviews/".$image; // storing image path to folder in root directory
      $db_path = "assets/images/reviews/".$image; // storing image path into database
  
    if(in_array($image_type, $extensions_arr)){
 
     // this method inserts the data into the database
     $queryrun = $Controller->insertReview($title,$review,$name,$db_path,$role);
...
$readqueryrun =  $Controller->readReview();
...
if (isset($_GET['deletedata'])){
  ...
  $id = $_GET['deletedata'];
  $table = 'tblreview';
  $deletequeryrun = $Controller->deleteReview($id, $table);
...
<td>
  <a href="gallery-data.php?deletedata=<?php echo $row['id'];?>" class="text-white mr-2 text-decoration-none btn btn-danger btn-sm" onclick="return confirm('Do you want to remove this data?')";
...
<div class="modal fade" id="adminadd">
	<div class="modal-dialog modal-lg">
	  <div class="modal-content">
		<div class="modal-header">
		  <h5 class="modal-title text-uppercase"><strong> Review Data Portal </strong> <i class="bi bi-people"></i></h5>
		</div>
    ...
    <div class="col-md-6">
        <div class="form-group mb-3">
        <input type="text" class="form-control" name="rname" placeholder="Enter full name">
        </div>
    </div> <!-- col 1 ends -->
    </div> <!-- row 1 ends -->
    <div class="row">
    <div class="col-md-6">
        <div class="form-group mb-3">
        <input type="text" class="form-control" name="title" placeholder="Title of Review">
        </div>
    </div> <!-- col 1 ends -->
    <div class="col-md-6">
        <div class="form-group mb-3">
    ...
    <div class="col-md-12">
        <div class="form-group mb-3">
        <textarea class="form-control" name="review" cols="6" rows="6">
      </textarea>
        </div>
    </div> <!-- col 1 ends -->
    </div> <!-- row 1 ends -->
